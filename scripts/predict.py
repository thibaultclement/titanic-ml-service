...
def main():
    ...
    df_model = select_model_features(df, include_target=True)

    test_df = df_model[df_model["Survived"].isna()].copy()
    X_test = test_df[features]

    predictions_df = make_predictions(
        model=pipeline, X_test=X_test, passenger_ids=test_df.index
    )

    Path("data/submissions").mkdir(parents=True, exist_ok=True)

    save_submission(predictions_df, filepath="data/submissions/submission.csv")
