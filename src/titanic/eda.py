...
def stats(df):
    ...
    stats.insert(
        stats.columns.get_loc("count") + 1,
        "null",
        null_counts.astype(str) + " (" + null_percentage_str + ")",
    )
...
def describe_numeric(df, col):
    ...
    spstats.probplot(df[col].dropna(), dist="norm", plot=axes[2])
...
def benchmark_age_figure(df):
    ...
    plt.title(
        "Density distribution of the different variables for filling in the null values of Age"
    )
    plt.legend()
    plt.show()
...
def benchmark_age_table(df):
    ...
    std_df = pd.DataFrame(
        {"Std": [StdAge, StdAgeMedian, StdAgeMedianTitle, StdAgeKNN, StdAgeETR]},
        index=["Age", "AgeMedian", "AgeMedianTitle", "AgeKNN", "AgeETR"],
    )
